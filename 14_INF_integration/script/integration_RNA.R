...
# Load data
seurat_obj <- readRDS("13_INF_QC_filtering/out/INF_seurat_obj_QC_filtered_list.rds")
...
ggsave("14_INF_integration/plot/UMAP_RNA_pre_integration_annotation.pdf", width = 12, height = 7)

DimPlot(seurat_obj, reduction = "umap", group.by = "sample") + 
  labs(title = "UMAP - RNA - pre integration") +
  theme(legend.text = element_text(size = 8))

ggsave("14_INF_integration/plot/UMAP_RNA_pre_integration_sample.pdf", width = 12, height = 7)
...
for (red in reductions){
  ...
  DimPlot(seurat_integrated, reduction = umap_reduction.name, group.by = "sample") +
    labs(title = glue("UMAP - post {reduction}")) + 
    theme(legend.text = element_text(size = 8))
  
  ggsave(glue("14_INF_integration/plot/{assay}/UMAP_{reduction}_sample.pdf"), 
         width = 8, 
         height = 7)
  
  DimPlot(seurat_integrated, reduction = umap_reduction.name, split.by = "sample", group.by = "annotation") +
    labs(title = glue("UMAP - post {reduction}")) + 
    theme(legend.text = element_text(size = 8))
  ...
  for (res in res_list){
    ...
    ggsave(glue(glue("14_INF_integration/plot/{assay}/UMAP_{reduction}_{assay}_snn_res_{res}.pdf")), 
           width = 8, 
           height = 7)
    
    DimPlot(seurat_integrated, reduction = umap_reduction.name, group.by = glue("{cluster.name}_res.{res}"), split.by = "sample", ncol = 3) +
      labs(title = glue("UMAP - post {reduction}"),
           subtitle = glue("{cluster.name}_res.{res}"))
    
    ggsave(glue("14_INF_integration/plot/{assay}/UMAP_{reduction}_{assay}_snn_res_{res}_split.by_sample.pdf"), 
           width = 12, 
           height = 8)
...
pdf(file = glue("14_INF_integration/plot/{assay}/clustree_{cluster.name}.pdf"), width = 12, height = 10)
clustree(seurat_integrated, assay = "RNA", return = "plot", prefix = glue("{cluster.name}_res."))
dev.off()
...
saveRDS(seurat_integrated, "14_INF_integration/out/INF_seurat_integrated_v5_RNA.rds")
...
# Reductions(obj_tmp)
# 
# filename <- glue("14_INF_integration/out/mydata_RNA_v5.h5Seurat")
