...
DimPlot(seurat_integrated, reduction = umap_reduction.name, group.by = glue("{cluster.name}_res.0.7"), label = TRUE) + NoLegend()

FeaturePlot(seurat_integrated, features = "Ptprc", reduction = umap_reduction.name)
FeaturePlot(seurat_integrated, features = "Trbc1", reduction = umap_reduction.name)
FeaturePlot(seurat_integrated, features = "Trbc2", reduction = umap_reduction.name)
grep("TRB", rownames(seurat_integrated), value = TRUE, ignore.case = TRUE) 

seurat_integrated$RNA_harmony_clusters_res.0.7

clusters_to_exclude <- c(7, 13, 16, 15, 17)
seurat_integrated_clean <- subset(seurat_integrated, subset = !(RNA_harmony_clusters_res.0.7 %in% clusters_to_exclude))

seurat_integrated_clean$RNA_harmony_clusters_res.0.7 %>% table()

DimPlot(seurat_integrated_clean, reduction = reduction, group.by = glue("{cluster.name}_res.0.7"), label = TRUE) + NoLegend()
...
seurat_integrated_clean$RNA_harmony_clusters_res.0.5

DimPlot(seurat_integrated_clean, reduction = reduction, group.by = "RNA_harmony_clusters_res.0.7")
DimPlot(seurat_integrated_clean, reduction = reduction, group.by = "orig.ident")

seurat_integrated_clean <- IntegrateLayers(
  object = seurat_integrated_clean, 
  method = HarmonyIntegration,
  orig.reduction = "pca", 
  new.reduction = "RNA_REintegrated.harmony",
  assay = "RNA", 
  verbose = FALSE
)
...
reduction <- "RNA_REintegrated.harmony"

seurat_integrated_clean <- FindNeighbors(seurat_integrated_clean, reduction = reduction, dims = 1:20)
res_list <- seq(0.1, 1.5, by = 0.1)
for (res in res_list){
  cluster.name <- glue("RNA_REharmony_clusters_res.{res}")
  seurat_integrated_clean <- FindClusters(seurat_integrated_clean, resolution = res, cluster.name = cluster.name)
}
seurat_integrated_clean <- RunUMAP(seurat_integrated_clean, reduction = reduction, dims = 1:20, reduction.name = "UMAP_RNA_REintegrated.harmony")
seurat_integrated_clean <- FindNeighbors(seurat_integrated_clean, reduction = reduction, dims = 1:20)

DimPlot(seurat_integrated_clean, reduction = "UMAP_RNA_REintegrated.harmony", group.by = "RNA_REharmony_clusters_res.0.7") 
DimPlot(seurat_integrated_clean, reduction = "UMAP_RNA_REintegrated.harmony", group.by = "orig.ident") 

umap_reduction.name <- "UMAP_RNA_REintegrated.harmony"
for (res in res_list){
  
  DimPlot(seurat_integrated_clean, reduction = umap_reduction.name, group.by = glue("RNA_REharmony_clusters_res.{res}"), label = TRUE) +
    labs(title = glue("UMAP - {reduction}"),
         subtitle = glue("RNA_REharmony_clusters_res.{res}"))
  
  ggsave(glue(glue("05_annotation/plot/re_integration/UMAP_RNA_REharmony_clusters_res.{res}.pdf")), 
         width = 8, 
         height = 7)
